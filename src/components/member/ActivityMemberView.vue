  ...
  <VContainer>
    <VRow>
        ...
        <VCol cols="12">
          ...
          :items="orders"
          :headers="headers"
          ><template #[`item.createdAt`]="{ item }">{{ new Date(item.createdAt). toLocaleString() }}</template><template #[`item.cart`]="{ item }"><ul><li
          v-for="cart in item.cart"
          :key="cart._id"
          >{{ cart.product.name }} * {{ cart.quantity }}</li></ul></template>
        </VDataTable>
        </VCol>
    </VRow>
</VContainer>
